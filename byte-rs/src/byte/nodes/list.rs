...
impl NodeList {
	...
	pub fn replace_by(&self, vec: Vec<Node>) {
...
impl Nodes {
	pub fn contains_delimiter_pair(nodes: &Vec<Node>, sta: &Symbol, end: &Symbol) -> bool {
	...
	pub fn split_ternary(
		nodes: &mut Vec<Node>,
		sta: &Symbol,
		end: &Symbol,
	) -> Option<(Vec<Node>, Vec<Node>, Vec<Node>)> {
	...
	pub fn map_nodes<P: FnMut(&Node) -> Option<Vec<Node>>>(nodes: &mut Vec<Node>, mut predicate: P) -> bool {
	...
	pub fn replace<P: FnMut(&Node) -> Option<Node>>(nodes: &mut Vec<Node>, mut replace: P) -> bool {
		let changed = {
			let mut changed = false;
	...
	pub fn replace_all(nodes: &mut Vec<Node>, new_nodes: Vec<Node>) -> bool {
		*nodes = new_nodes;
		true
	}

	pub fn split_by<P: FnMut(&Node) -> bool, S: FnMut(NodeList) -> Node>(
		scope: &Scope,
		nodes: &mut Vec<Node>,
		mut split: P,
		mut node: S,
	...
	pub fn split_by_items<P: FnMut(&Node) -> bool>(
		scope: &Scope,
		nodes: &mut Vec<Node>,
		mut split: P,
	) -> Vec<NodeList> {
	...
	pub fn get_symbol(nodes: &Vec<Node>, index: usize) -> Option<Symbol> {
		nodes.get(index).and_then(|x| x.symbol())
	}

	pub fn test_at<P: FnOnce(&Node) -> bool>(nodes: &Vec<Node>, index: usize, predicate: P) -> bool {
		nodes.get(index).map(|x| predicate(x)).unwrap_or(false)
	}

	pub fn is_identifier(nodes: &Vec<Node>, index: usize) -> bool {
		Self::test_at(nodes, index, |x| matches!(x.bit(), Bit::Token(Token::Word(..))))
	}

	pub fn is_keyword(nodes: &Vec<Node>, index: usize, word: &Symbol) -> bool {
		Self::test_at(nodes, index, |x| x.is_word(word))
	}

	pub fn is_symbol(nodes: &Vec<Node>, index: usize, symbol: &Symbol) -> bool {
		Self::test_at(nodes, index, |x| x.is_symbol(symbol))
	}
}
